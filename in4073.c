				...
				if(CRCIsValid == true){
					switch(headerByte){
						...
						case J_CONTROL:
							...
							break;
						// Parsing Keyboard input packets
						case K_LIFT:
...
void check_battery(){
	...
	//Do not allow mode transitions if battery voltage is lower than 10.5 volts
	if((QuadState == SAFE && QuadState == SAFE_NONZERO) && (bat_volt <= 1050)) {
		printf("Battery Critically low (%d volts)!!\n",bat_volt);
		low_battery=true;
	}
	
	//Give user warning about low battery
	...
	//Move to panic mode if battery voltage goes below 10.5 volts 
	else{
		printf("Battery critically low(%d volts)!! Please change the battery and restart......\n",bat_volt);
		low_battery=true; 
